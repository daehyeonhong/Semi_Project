			...
			<table class="table table-hover">
				...
				<tr>
					...
					<td><input type="number" id="qty<%=i%>"
						value="<%=product.getQuantity()%>" onchange="k(<%=i%>)"> <input
						type="button" class="btn btn-primary" onclick="change(<%=i%>)"
						value="수정" id="btn<%=i%>" disabled="disabled" /></td>
					<td><%=total%></td>
...
int sum=0;
List<Product> cartList = (List<Product>)session.getAttribute("cartlist");
if(cartList==null||(cartList.size()==0)) cartList =new ArrayList<Product>();
