...
$('#btn_mail').click(function () {
  ...
  function checkForm(){
		var checked1=$('#idChecked').val();/* document.getElementById('idChecked').value; */
		var checked2=$('#mailChecked').val();/* document.getElelmentById('mailChecked').value;	 */
		alert(checked1);
		alert(checked2);
		if(checked1=='false'){
			alert('아이디 체크를 해주세요');
			...
			alert('메일 인증 해주세요');
			return false;
		} else if (checked1 == 'true' && checked2=='1'){
			checkForm.submit();
		}
}   
		...
		<div class="context" id="context1">
			...
			<div class="container">
				<form name="newMember" class="form-horizontal" id="newMember"
					action="./processAddMember.jsp" method="post" onsubmit="return checkForm()">
					<div class="form-group row">
						<label class="col-sm-2">아이디</label>
						<div class="col-sm-3">

							<input name="id" id="id" class="form-control" placeholder="id">
							<input type="button" value="중복확인" class="btn btn-primary" id="dupCheck" required> <input
								type="hidden" id="idChecked" name="idChecked" value='false'>
					...
					<div class="form-group row">
						...
						<div class="col-sm-10">
							...
							</select> <input type="button" id="btn_mail" value="이메일 인증">
							<!-- onclick='btnEmail()' -->
							<input type="hidden" id="mailChecked" name="mailChecked" value="" />
							<label id="result2" class="badge badge-success"></label>
						</div>
					</div>
					<div class="form-group row">
					...
					<div class="form-group row">
						<div class="container" style="width: 700px;">
							<button type="submit" class="btn btn-success btn-lg" id="btn"
								value="등록" onclick='btn()'>회원가입 완료</button>
							<button type="reset" class="btn btn btn-lg" id="btn_cancel"
								value="취소" onclick='location.href="../welcome.jsp"'>회원가입
								취소</button>
						</div>
					</div>
				</form>
