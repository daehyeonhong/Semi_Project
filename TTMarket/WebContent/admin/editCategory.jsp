...
<body>
	...
	if (resultSet.next()) {
	%>
	<form action="./processEditCategory.jsp" id="categoryForm"
		method="post">
		<table class="table table-hover table-bordered col-sm-9">
			...
			<tr>
				<td><%=resultSet.getString(1)%><input type="hidden" name="seq"
					value="<%=seq%>" /></td>
				<td>
					<input placeholder="CategoryName" id="categoryName" name="categoryName" value="<%=resultSet.getString(2)%>" />
				</td>
				<td>
					<input placeholder="Description" id="description" name="description" value="<%=resultSet.getString(3)%>" />
				</td>
			</tr>
		</table>
		<div class="col-sm-9 text-right">
			<input class="btn btn-warning" type="button" value="취소" onclick="cancelBtn()" />
			<input class="btn btn-danger" type="button" value="삭제" onclick="removeBtn(<%=seq%>)" />
