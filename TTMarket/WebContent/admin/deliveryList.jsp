...
<body>
	<%
		String searchSql = "select s.seq,s.saleDate,s.productId,s.saleQty,d.name,d.deliveryDate,d.nation,d.zipCode,d.address, s.status from ttsale s,ttdelivery d where s.sessionId=d.sessionId and s.status not in (5) group by s.seq order by s.seq,d.deliveryDate,s.productId";
...
<body>
	<%
		String searchSql = "select s.saleDate,s.productId,s.saleQty,d.name,d.deliveryDate,d.nation,d.zipCode,d.address, s.status,d.seq from ttsale s,ttdelivery d where s.sessionId=d.sessionId and s.status not in (5) group by s.seq order by s.seq,d.deliveryDate,s.productId";
	PreparedStatement searchPreparedStatement = con.prepareStatement(searchSql);
	ResultSet searchResultSet = searchPreparedStatement.executeQuery();
	String satusSql = "select status from ttstatus";
	PreparedStatement statusPreparedStatement = con.prepareStatement(satusSql);
